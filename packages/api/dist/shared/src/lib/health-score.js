...
const ClickHouseBacklinkRepository_1 = require("./clickhouse/repositories/ClickHouseBacklinkRepository");
const ClickHouseUxSessionStore_1 = require("./clickhouse/repositories/ClickHouseUxSessionStore");
const ClickHouseHealthScoreRepository_1 = require("./clickhouse/repositories/ClickHouseHealthScoreRepository");
const ContentDepthService_1 = require("./services/ContentDepthService");
const TruthRiskService_1 = require("./services/TruthRiskService");
const UXFrictionService_1 = require("./services/UXFrictionService");
class HealthScoreService {
    static async calculateAndSave(siteId) {
        ...
        const backlinks = await ClickHouseBacklinkRepository_1.ClickHouseBacklinkRepository.getBacklinks(siteId, 10000); // Fetch all/many backlinks
        console.log(`Fetched ${backlinks.length} backlinks for ${siteId}`);
        const uxMetrics = await ClickHouseUxSessionStore_1.ClickHouseUxSessionStore.getMetrics(siteId);
        // Calculate max TSPR for normalization
        const maxTspr = Math.max(...pages.map((p) => p.tspr || 0)) || 1;
        const scores = [];
        for (const page of pages) {
            // Filter backlinks for this page
            const pageBacklinks = backlinks.filter((b) => b.target_url === page.url);
            // 1. Content Depth
        ...
        await ClickHouseHealthScoreRepository_1.ClickHouseHealthScoreRepository.saveScores(scores);
        return scores;
    }
    static getSuggestion(score, components) {
        const { tspr, content, ux, truth, backlinks } = components;
        if (score >= 80)
            return "✅ Maintain";
        if (score < 40) {
            if (tspr > 80 && content < 40)
                return "Rewrite for better coverage";
            if (content > 80 && ux > 70)
                return "Improve navigation (UX)";
            if (truth > 60)
                return "Review fact-checking"; // High risk score means low component score? Wait, truth component is INVERSE risk. So low component = high risk.
            // Let's correct: truth component is 1 - risk. So if truth component < 40 (risk > 60).
            if (truth < 40)
                return "Review fact-checking";
            if (backlinks < 30)
                return "Focus on link-building";
        }
        if (content < 50)
