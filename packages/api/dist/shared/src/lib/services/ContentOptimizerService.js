...
class ContentOptimizerService {
     ...
     */
    static async scoreContent(text, targetKeyword, siteId) {
        const suggestions = [];
        ...
        const overall = Math.round((wordCountScore * 0.25) +
            (keywordScore * 0.25) +
     ...
     */
    static async getSerpRecommendations(keyword, siteId) {
    ...
    static getReadingGrade(fleschScore) {
        if (fleschScore >= 90)
            return '5th grade';
        if (fleschScore >= 80)
            return '6th grade';
    ...
    static extractCommonHeaders(pages) {
    ...
    static extractCommonTerms(pages, excludeKeyword) {
        ...
        for (const page of pages) {
            if (page.text) {
                const words = page.text.toLowerCase()
                    .split(/\s+/)
                    .filter((w) => w.length > 3 && !stopWords.has(w) && w !== excludeKeyword.toLowerCase());
                for (const word of words) {
