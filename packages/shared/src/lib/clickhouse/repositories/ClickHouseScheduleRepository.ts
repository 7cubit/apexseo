...
export class ClickHouseScheduleRepository {
    ...
    static async getSchedules(projectId: string) {
        if (!client) {
    ...
    static async toggleSchedule(projectId: string, agentName: string, enabled: boolean) {
        if (!client) return;
