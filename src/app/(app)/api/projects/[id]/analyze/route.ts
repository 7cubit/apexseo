...
import { SiteAnalysisWorkflow, QUEUE_ORCHESTRATOR } from '@/temporal/workflows';
...
export async function POST(
    ...
    try {
        ...
        const handle = await client.workflow.start(SiteAnalysisWorkflow, {
            taskQueue: QUEUE_ORCHESTRATOR,
            workflowId: `analysis-${projectId}-${Date.now()}`,
            args: [{ projectId, siteUrl, maxPages: 100 }],
        });
